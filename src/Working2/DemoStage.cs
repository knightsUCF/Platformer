namespace MoenenVoxel {
	...
	public class DemoStage : MonoBehaviour {
		...
		public Vector3 PlayerStartPos = new Vector3(0.0f, 10.0f, 0.0f);
		public Vector3 respawnPos = new Vector3(0.0f, 10.0f, 0.0f);
		...
		void GameStart () {
			...
			// drop the player in from above

			// CurrentPlayer.position = Vector3.zero;

			// CurrentPlayer.position = new Vector3(0, 4, 0);

			CurrentPlayer.position = PlayerStartPos;
		...
		void Update () {
			...
			if (CrossPlatformInputManager.GetButtonDown("Fire1") && FirstStart)
			{
				GameStart();
			}
...
Press <size=50><color=#cc3333ff>[ESC]</color></size> to Continue",
			...
			// Change Player
			if (Input.GetKeyDown(KeyCode.Tab)) { // || CrossPlatformInputManager.GetButtonDown("Fire1")) {
				currentPlayerID++;
			...
			*/
			...
			if (CurrentPlayer && CurrentPlayer.position.y < MainGround.position.y - 2f) {
				...
				CurrentPlayer.position = respawnPos;
			}
